<template>
  <div class="user">
    ...
    <!-- 弹出框 -->
    <popup-box :show="popup.show" :title="popup.title" @on-change="onChangePopup">
      <UserAdd v-if="popup.type == 1" @on-submit="onAddSubmit"/>
      <UserEdit v-else-if="popup.type==2" :userData="detailInfo" @on-submit="onEditSubmit" />
    ...
    const getList = async () => {
      ...
      if (getApiUserList?.data?.list?.length) {
        getApiUserList?.data?.list.map((item) => {
          item.activate_time = item.activate_time ? moment(new Date(item.activate_time)).format('YYYY-MM-DD HH:mm:ss') : ''
          item.locked_time = item.locked_time ?moment(new Date(item.locked_time)).format('YYYY-MM-DD HH:mm:ss') : ''
          item.disabled_time = item.disabled_time ?moment(new Date(item.disabled_time)).format('YYYY-MM-DD HH:mm:ss') : ''
          item.last_login_time = item.last_login_time ?moment(new Date(item.last_login_time)).format('YYYY-MM-DD HH:mm:ss') : ''
          item.create_time = item.create_time ?moment(new Date(item.create_time)).format('YYYY-MM-DD HH:mm:ss') : ''
          return item;
        })
